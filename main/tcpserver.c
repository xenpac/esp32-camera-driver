        ...
        if (!strcmp(uri,"/download"))
        {
            if (!IsStreaming) //if currently streaming 0 Bytes will be downloaded!
            {
                ...
                if (flashlight)
                {
                    gpio_set_level(4, 1); // turn led on
                    vTaskDelay(400/portTICK_PERIOD_MS);  // wait a little to get camera exposure settle to new light conditions
                    I2S_Stop(); // we want the latest picture. this will cause ringbuffer init
                }
...
int get_frame(uint8_t **buf, size_t *len)
{
    ...
    i2s_unlockfb(); //free last used framebufer
    fb = i2s_getframe(); // get a new framebuffer with current picture data
    if (!fb)
    {
...
static uint16_t set_register(char *uri)
{
    ...
    s = get_camstruct(); // get the cameras function list
...
//check 'variable' for the request parameter
    ...
    {
        sprintf(iobuf,"- NetFPS:%d CamFPS:%d I2sFPS:%d - DMAerrors:%d JPGerrors:%d - UpTime(hrs):%d",NetFPS,HwFPS,I2sFPS,DMAerrors,JPGerrors,(uptime/3600));
...
static int get_camstatus(void)
{
    sensor_t *s  = get_camstruct(); // get the status of camera controls from camera
    if (s == NULL) return 0;
    char *p = iobuf;
...
void stream_speed(int full)
{
    sensor_t *s  = get_camstruct(); // get the cameras function list
